...
class BundlesController extends Controller
{
    ...
    public function all()
    {
        ...
        return view('frontend.bundles.index', compact('bundles', 'purchased_bundles', 'recent_news', 'featured_formations', 'categories'));
    }
    ...
    public function show($bundle_slug)
    {
        ...
        $formations = $bundle->formations()->orderby('id', 'asc')->get();


        return view('frontend.bundles.show', compact('bundle', 'purchased_bundle', 'recent_news', 'bundle_rating', 'bundle_rating', 'formations', 'total_ratings', 'is_reviewed'));
    }


    public function rating($formation_id, Request $request)
    {
        $bundle = Bundle::findOrFail($formation_id);
    ...
    public function getByCategory(Request $request)
    {
        ...
        if ($category != "") {
            ...
            $formations = $category->formations()->where('published', '=', 1)->paginate(9);
            return view('frontend.formations.index', compact('formations', 'category', 'recent_news', 'featured_formations'));
        }
        return abort(404);
    }
