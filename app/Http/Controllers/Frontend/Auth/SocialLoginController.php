 ...
 */
class SocialLoginController extends FrontendBaseController
{
    /**
     * @var UserRepository
     */
    ...
    public function login(Request $request, $provider)
    {
        ...
        if (!in_array($provider, $this->socialiteHelper->getAcceptedProviders())) {
            return redirect()->route(home_route())->with('danger', __('auth.socialite.unacceptable', ['provider' => $provider]));
        }
        ...
        try {
            ...
            return redirect()->route(home_route())->with('danger', $e->getMessage());
        }

        if (is_null($user)) {
            return redirect()->route(home_route())->with('danger', __('exceptions.frontend.auth.unknown'));
        }
