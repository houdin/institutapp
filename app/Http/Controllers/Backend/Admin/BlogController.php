...
class BlogController extends BackendBaseController
{
    ...
    public function store(StoreBlogsRequest $request)
    {
        ...
        $extension = last(explode('.', $request->featured_image));
        $name = head(explode('.', $request->featured_image));

        $image = ModelImage::create(
            [
                'name' => $request->featured_image,
                'url' => asset('storage/uploads/' . date('Y/m/') . $request->featured_image),
                'file_name' => $name,
            ]
        );
        ...
        if ($blog->id) {
            return redirect()->route('admin.blogs.index')->with('success', __('alerts.backend.general.created'));
        } else {
            return redirect()->route('admin.blogs.index')->with('danger', __('alerts.backend.general.error'));
        }
    }
    ...
    public function update(UpdateBlogsRequest $request, $id)
    {
        ...
        return redirect()->route('admin.blogs.index')->with('success', __('alerts.backend.general.updated'));
    }
    ...
    public function destroy($id)
    {
        ...
        $blog = Blog::findOrfail($id);
        $blog->delete();
        return redirect()->route('admin.blogs.index')->with('success', __('alerts.backend.general.deleted'));
    }
