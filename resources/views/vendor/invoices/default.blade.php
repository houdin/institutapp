...
<div style="display: inline-block;clear: both;width: 100%;">
    ...
    <div style="float: right; width:200pt;height: auto;display: inline-block">
        ...
        <div class="panel panel-default" style="padding: 15px;padding-top: 0px">
            {!! count($invoice->customer_details) == 0 ? '<i>No customer details</i><br />' : '' !!}
            <h4 style="font-weight: bold; font-family: DejaVu Sans;"> {{ $invoice->customer_details['name'] }}</h4>
            <span>Email :</span> {{ $invoice->customer_details['email'] }}
        </div>
    </div>
</div>
