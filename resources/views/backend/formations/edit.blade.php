        ...
        <div class="card-body">
            ...
            @if (Auth::user()->isAdmin())
                <div class="row">
                    ...
                    <div class="col-10 form-group">
                        ...
                        {!! Form::select('teachers[]', $teachers, old('teachers') ? old('teachers') : $formation->teachers->pluck('id')->toArray(), ['class' => 'form-control select2', 'multiple' => 'multiple', 'required' => true]) !!}
                    </div>
            ...
            <div class="row">
                ...
                <div class="col-12 col-lg-4 form-group">
                    ...
                    {!! Form::label('formation_image', trans('backend/labels.backend.formations.fields.formation_image'), ['class' => 'control-label', 'accept' => 'image/jpeg,image/gif,image/png']) !!}
                    {!! Form::file('formation_image', ['class' => 'form-control']) !!}
                    {!! Form::hidden('formation_image_max_size', 8) !!}
                    {!! Form::hidden('formation_image_max_width', 2700) !!}
                    {!! Form::hidden('formation_image_max_height', 1800) !!}
                    @if ($formation->image)
                        <a href="{{ $formation->image->featuredImageUrl(5) }}" target="_blank"><img height="50px"
                                src="{{ $formation->image->featuredImageUrl(1) }}" class="mt-1"></a>
                    @endif
                </div>
                <div class="col-12 col-lg-4 form-group">
            ...
            <div class="row">
                <div class="col-md-12 form-group">
                    ...
                    {!! Form::select('media_type', ['youtube' => 'Youtube', 'vimeo' => 'Vimeo', 'upload' => 'Upload', 'embed' => 'Embed'], $formation->mediavideo ? $formation->mediavideo->type : null, ['class' => 'form-control', 'placeholder' => 'Select One', 'id' => 'media_type']) !!}


                    {!! Form::text('video', $formation->mediavideo ? $formation->mediavideo->url : null, ['class' => 'form-control mt-3 d-none', 'placeholder' => trans('backend/labels.backend.modules.enter_video_url'), 'id' => 'video']) !!}
                    ...
                    <input type="hidden" name="old_video_file"
                        value="{{ $formation->mediavideo && $formation->mediavideo->type == 'upload' ? $formation->mediavideo->url : '' }}">
                    @if ($formation->mediavideo != null)
                        <div class="form-group">
                            <a href="#" data-media-id="{{ $formation->mediaVideo->id }}"
                    ...
                    @if ($formation->mediavideo && $formation->mediavideo->type == 'upload')
                        <video width="300" class="mt-2 d-none video-player" controls>
                            <source
                                src="{{ $formation->mediavideo && $formation->mediavideo->type == 'upload' ? $formation->mediavideo->url : '' }}"
                                type="video/mp4">
...
@push('after-scripts')
    <script>
        ...
        @if ($formation->mediavideo)
            @if ($formation->mediavideo->type != 'upload')
