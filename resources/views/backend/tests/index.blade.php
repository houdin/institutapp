        ...
        <div class="card-header">
            ...
            @can('test_create')
                <div class="float-right">
                    <a href="{{ route('admin.tests.create') }}" class="btn btn-success">@lang('backend/strings.backend.general.app_add_new')</a>
        ...
        <div class="card-body table-responsive">
            <div class="row">
                <div class="col-12 col-lg-6 form-group">
                    {!! Form::label('formation_id', trans('backend/labels.backend.modules.fields.formation'), [
                        'class' => 'control-label',
                    ]) !!}
                    {!! Form::select(
                        'formation_id',
                        $formations,
                        request('formation_id') ? request('formation_id') : old('formation_id'),
                        ['class' => 'form-control js-example-placeholder-single select2 ', 'id' => 'formation_id'],
                    ) !!}
                </div>
            </div>
            <div class="d-block">
...
@push('after-scripts')
    <script>
        $(document).ready(function() {
            ...
            @can('test_delete')
                @if (request('show_deleted') != 1)
                    $('.actions').html('<a href="' + '{{ route('admin.tests.mass_destroy') }}' +
