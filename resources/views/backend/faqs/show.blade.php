...
@section('title', __('backend/labels.backend.faqs.title').' | '.app_name())

@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="page-title float-left mb-0">@lang('backend/labels.backend.questions.title')</h3>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-12">
                    <table class="table table-bordered table-striped">
                        <tr>
                            <th>@lang('backend/labels.backend.questions.fields.question')</th>
                            <td>{!! $question->question !!}</td>
                        </tr>
                        <tr>
                            <th>@lang('backend/labels.backend.questions.fields.question_image')</th>
                        ...
                        <tr>
                            <th>@lang('backend/labels.backend.questions.fields.score')</th>
            ...
            <div class="tab-content">
                ...
                <div role="tabpanel" class="tab-pane container active" id="questionsoptions">
                    <table class="table table-bordered table-striped {{ count($questions_options) > 0 ? 'datatable' : '' }}">
                        <thead>
                        <tr>
                            <th>@lang('backend/labels.backend.questions_options.fields.question')</th>
                            <th>@lang('backend/labels.backend.questions_options.fields.option_text')</th>
                            <th>@lang('backend/labels.backend.questions_options.fields.correct')</th>
                                        ...
                                        <td>
                                            ...
                                            <a data-method="delete" data-trans-button-cancel="Cancel"
                                               ...
                                               onclick="$(this).find('form').submit();">
                                                {{trans('backend/strings.backend.general.app_restore')}}
                                            ...
                                            <a data-method="delete" data-trans-button-cancel="Cancel"
                                               ...
                                               onclick="$(this).find('form').submit();">
                                                {{trans('backend/strings.backend.general.app_permadel')}}
                        ...
                        @else
                            <tr>
                                <td colspan="7">@lang('backend/strings.backend.general.app_no_entries_in_table')</td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
                ...
                <div role="tabpanel" class="tab-pane container" id="tests">
                    <table class="table table-bordered table-striped {{ count($tests) > 0 ? 'datatable' : '' }}">
                        ...
                        <tr>
                            ...
                            <th>@lang('backend/labels.backend.tests.fields.module')</th>
                        ...
                        @if (count($tests) > 0)
                            @foreach ($tests as $test)
                                <tr data-entry-id="{{ $test->id }}">
                                    <td>{{ ($test->formation) ? $test->formation->title : '' }}</td>
                                    <td>{{ ($test->module) ? $test->module->title : '' }}</td>
                                    <td>{{ $test->title }}</td>
                                    <td>
                        ...
                        @else
                            <tr>
                                <td colspan="10">@lang('backend/strings.backend.general.app_no_entries_in_table')</td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
            ...
            <a href="{{ route('admin.questions.index') }}"
               class="btn btn-default border mt-3">@lang('backend/strings.backend.general.app_back_to_list')</a>
        </div>
    </div>
@stop
