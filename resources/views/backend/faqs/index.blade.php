...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="page-title float-left mb-0">@lang('backend/labels.backend.faqs.title')</h3>
            ...
            <table id="myTable"
                ...
                <tr>
                    ...
                    @endcan
                        ...
                        <th>@lang('backend/labels.backend.faqs.fields.category')</th>
                    <th>@lang('backend/labels.backend.faqs.fields.question')</th>
                    <th>@lang('backend/labels.backend.faqs.fields.answer')</th>
                    <th>@lang('backend/labels.backend.faqs.fields.status')</th>
                    @if( request('show_deleted') == 1 )
...
@push('after-scripts')
    <script>
        ...
        $(document).on('click', '.switch-input', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.faqs.status', ':id') }}".replace(':id', id),
                data: {
                    _token:'{{ csrf_token() }}',
                    id: id,
                },
