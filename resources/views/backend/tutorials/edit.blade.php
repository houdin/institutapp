...
@section('content')
    ...
    {!! Form::model($tutorial, ['method' => 'PUT', 'route' => ['admin.tutorials.update', $tutorial->id], 'files' => true,]) !!}

    <div class="card">
        <div class="card-header">
            <h3 class="page-title float-left mb-0">@lang('backend/labels.backend.formations.edit')</h3>
        ...
        <div class="card-body">
            ...
            @if (Auth::user()->isAdmin())
                <div class="row">
                    ...
                    <div class="col-10 form-group">
                        ...
                        {!! Form::select('teachers[]', $teachers, old('teachers') ? old('teachers') : $tutorial->teachers->pluck('id')->toArray(), ['class' => 'form-control select2', 'multiple' => 'multiple','required' => true]) !!}
                    </div>
            ...
            <div class="row">
                ...
                <div class="col-12 col-lg-4 form-group">
                    ...
                    {!! Form::label('tutorial_image', trans('backend/labels.backend.formations.fields.formation_image'), ['class' => 'control-label','accept' => 'image/jpeg,image/gif,image/png']) !!}
                    {!! Form::file('tutorial_image', ['class' => 'form-control']) !!}
                    {!! Form::hidden('tutorial_image_max_size', 8) !!}
                    {!! Form::hidden('tutorial_image_max_width', 2700) !!}
                    {!! Form::hidden('tutorial_image_max_height', 1800) !!}
                    @if ($tutorial->image)
                        <a href="{{ featured_image_url($tutorial, 1) }}" target="_blank"><img
                                    height="50px" src="{{ featured_image_url($tutorial, 1) }}"
                                    class="mt-1"></a>
                    @endif
                </div>
                {{-- <div class="col-12 col-lg-4 form-group">
                    {!! Form::label('start_date', trans('backend/labels.backend.formations.fields.start_date').' (yyyy-mm-dd)', ['class' => 'control-label']) !!}
                    {!! Form::text('start_date', old('start_date'), ['class' => 'form-control date', 'pattern' => '(?:19|20)[0-9]{2}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-9])|(?:(?!02)(?:0[1-9]|1[0-2])-(?:30))|(?:(?:0[13578]|1[02])-31))', 'placeholder' => trans('backend/labels.backend.formations.fields.start_date').' (Ex . 2019-01-01)']) !!}
            ...
            <div class="row">
                <div class="col-md-12 form-group">
                    ...
                    {!! Form::select('media_type', ['youtube' => 'Youtube','vimeo' => 'Vimeo','upload' => 'Upload','embed' => 'Embed'],($tutorial->mediavideo) ? $tutorial->mediavideo->type : null,['class' => 'form-control', 'placeholder' => 'Select One','id'=>'media_type' ]) !!}


                    {!! Form::text('video', ($tutorial->mediavideo) ? $tutorial->mediavideo->url : null, ['class' => 'form-control mt-3 d-none', 'placeholder' => trans('backend/labels.backend.modules.enter_video_url'),'id'=>'video'  ]) !!}
                    ...
                    <input type="hidden" name="old_video_file"
                           value="{{($tutorial->mediavideo && $tutorial->mediavideo->type == 'upload') ? $tutorial->mediavideo->url  : ""}}">
                    @if($tutorial->mediavideo != null)
                        <div class="form-group">
                            <a href="#" data-media-id="{{$tutorial->mediaVideo->id}}"
                    ...
                    @if($tutorial->mediavideo && ($tutorial->mediavideo->type == 'upload'))
                        <video width="300" class="mt-2 d-none video-player" controls>
                            <source src="{{($tutorial->mediavideo && $tutorial->mediavideo->type == 'upload') ? $tutorial->mediavideo->url  : ""}}"
                                    type="video/mp4">
...
@push('after-scripts')
    <script>
        ...
        @if($tutorial->mediavideo->type !=  'upload')
