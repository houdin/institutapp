            ...
            <div class="table-responsive">
                <div class="d-block">
                    <ul class="list-inline">
                        ...
                        |
                        <li class="list-inline-item">
                            <a href="{{ route('admin.tutorials.index') }}?show_deleted=1"
                               style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">{{trans('backend/labels.general.trash')}}</a>
                        </li>
                    </ul>
                </div>

                <table id="myTable" class="table table-bordered table-striped @can('tutorial_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan" >
                    <thead>
                    <tr>
                        @can('tutorial_delete')
                        ...
                        <th>@lang('backend/labels.backend.formations.fields.price') <br><small>(in {{$appCurrency['symbol']}})</small></th>
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function () {
            var route = '{{route('admin.tutorials.get_data')}}';

            @if(request('show_deleted') == 1)
                route = '{{route('admin.tutorials.get_data',['show_deleted' => 1])}}';
            @endif

            @if(request('teacher_id') != "")
                route = '{{route('admin.tutorials.get_data',['teacher_id' => request('teacher_id')])}}';
            @endif

            @if(request('cat_id') != "")
                route = '{{route('admin.tutorials.get_data',['cat_id' => request('cat_id')])}}';
            @endif
            ...
            {{--@can('tutorial_delete')--}}
            {{--@if(request('show_deleted') != 1)--}}
            {{--$('.actions').html('<a href="' + '{{ route('admin.tutorials.mass_destroy') }}' + '" class="btn btn-xs btn-danger js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">Delete selected</a>');--}}
            {{--@endif--}}
            {{--@endcan--}}
        });
