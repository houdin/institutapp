                    ...
                    <v-card-text>
                        ...
                        <v-content v-if="uploadInfo.uploads">
                            <v-text-field v-model="search" append-icon="search" label="Search" single-line hide-details
                                box></v-text-field>
                            <v-data-table :headers="headers" :items="uploadInfo.uploads" :search="search"
                                :pagination.sync="pagination" hide-actions no-data-text="No files have been uploaded">
                                <template v-slot:items="props">
                                    <td><a :href="props.item.filepath">{{ $filters.truncate(props.item.name, 150) }}</a>
                                    </td>
                                    <td width="15%">{{ props.item.created_at }}</td>
                                    <td width="25%">
                                        <v-form method="POST" id="deleteForm"
                                            @submit.prevent="deleteFile(props.item.id)">
                                            <v-btn small type="submit" color="red darken-1" class="white--text">Delete
                                            </v-btn>
                                        </v-form>
                                    </td>
                                </template>
            ...
            <v-form method="POST" id="taskForm" @submit.prevent="uploadFile" ref="form" lazy-validation>
                <v-card>
                    ...
                    <v-container grid-list-sm class="pa-4">
                        <v-layout row wrap>
                            <v-flex xs12>
                                <v-text-field label="Upload a File" @click="selectFile" v-model="uploadFileName"
        ...
        uploadFile() {
            if (this.file != '') {
                ...
                axios.post('/api/uploads/' + this.uploadType + '/' + this.uploadInfo.id, fileUploadForm, {
                    ...
                    .then(response => {
                        Inertia.on('dataRefresh', this.uploadInfo.id)
                    ...
                    .catch(function (error) {
                        Inertia.on('error', error.data.message)
                    })
            }
        },
        deleteFile(file) {
