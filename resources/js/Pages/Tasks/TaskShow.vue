        ...
        <div v-if="issue.id">
            <v-container fluid grid-list-md>
                <v-layout row>
                    <v-flex xs12>
                        <v-card>
                            <v-alert :value="true" v-if="issue.resolved" type="success"
                                @click="unresolveIssue(issue.id)" class="clickable">
                                Issue resolved {{ issue.resolved_date }}.
                            </v-alert>
                            <v-alert :value="true" v-else type="warning" @click="resolveIssue(issue.id)"
                                class="clickable">
                ...
                <v-form method="POST" id="editIssueForm" @submit.prevent="updateIssue" ref="form" lazy-validation>
                    <v-card>
                        ...
                        <v-container grid-list-sm class="pa-4">
                            <v-layout row wrap>
                                <v-flex xs12>
                                    <v-textarea prepend-icon="notes" label="Description" v-model="issue.description"
                                        :rules="[v => !!v || 'Description is required']" required></v-textarea>
                                </v-flex>
                                <v-flex xs12>
                                    <v-autocomplete :items="projects" item-text="name" item-value="id"
                                        label="Select a project" prepend-icon="work" v-model="issue.project_id">
                                    </v-autocomplete>
                                </v-flex>
                                <v-flex xs12>
                ...
                <v-layout row>
                    <v-flex xs12 class="text-sm-center mt-3">
                        <p class="headline">
                            ...
                            Sorry, we could not load this issue.
                        </p>
                        <p class="subheading">This issue either does not exist or you do not have access to it.</p>

                        <v-btn type="submit" flat color="blue darken-2" to="/issues">
