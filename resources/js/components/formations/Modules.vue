      ...
      <div class="formation-details-item border-bottom-0 mb-0">
        ...
        <template v-if="test_exists">
          ...
          <template v-if="test_result !== null">
            ...
            <Form v-if="$filters.config('retest')" @submit="onSubmit" :validation-schema="schema" id="loginForm" class="contact_form">
            ...
            <template v-if="module.question.length > 0">
              ...
              <template v-for="(question, index) in module.question" :key="index">
                ...
                <h4 class="mb-0">{{ index+1 }}
                  . {{  question.question  }}
                  <small v-if="isAttempted(question.id, test_result.id)" class="badge badge-danger"> {{ trans.get('labels.frontend.formation.not_attempted') }}</small>
                </h4>
                <br />
                <ul class="options-list ps-4">

                  <li v-for="(option, index) in question.options" :key="index" :class="((answered(option.id, test_result.id) != null && answered(option.id, test_result.id) == 1) || (option.correct == true)) ? 'correct' : ((answered(option.id, test_result.id) != null && answered(option.id, test_result.id) == 2) ? 'incorrect' :''  )"> {{ option.option_text }}
          ...
          <div v-else class="test-form">
            <Form v-if="module.questions.length > 0" @submit="onSubmit" :validation-schema="schema" id="loginForm" class="contact_form">
              ...
              <template v-for="(question, index) in module.questions" :key="index">
                ...
                <div v-for="(option, ind) in question.options" :key="ind" class="form-group">
                  ...
                  <div class="mb-2">
                    ...
                    <Field id="email" type="radio" :name="questions[ question.id ]" class="form-control mb-0 ps-5" :value="option.id" v-model="email"></Field>
                ...
                <br />
                <input class="btn gradient-bg text-white font-weight-bold" type="submit" value=" {{ trans.get('labels.frontend.formation.submit_results') }} " />
              </template>
            </Form>
        ...
        <div v-if="module.mediaVideo && module.mediavideo.count > 0" class="formation-single-text">
          <div v-if="module.mediavideo != ''" class="formation-details-content mt-3">
            <div class="video-container mb-5" :data-id="module.mediavideo.id">
              ...
              <div v-if="module.mediavideo.type == 'youtube'" id="player" class="js-player" data-plyr-provider="youtube" :data-plyr-embed-id="module.mediavideo.file_name"></div>
              <div v-else-if="module.mediavideo.type == 'vimeo'" id="player" class="js-player" data-plyr-provider="vimeo" :data-plyr-embed-id="module.mediavideo.file_name"></div>
              <video v-else-if="module.mediavideo.type == 'upload'" poster="" id="player" class="js-player" playsinline controls>
        ...
        <div v-if="(module.downloadableMedia != '') && (module.downloadableMedia.count > 0)" class="formation-single-text mt-4 px-3 py-1 gradient-bg text-white">
          <div class="formation-title mt10 headline position-relative">
            <h4 class="text-white">
          ...
          <div v-for="(media, index) in module.downloadableMedia" :key="index" class="formation-details-content text-white">
            <p class="form-group">
              <router-link :to="{name: 'download', params: {filename : media.name, module : module.id}}" class="text-white font-weight-bold"><i class="fa fa-download"></i> {{ media.name }}
                ({{ $filters.Utility.number_format(parseFloat(media.size) / 1024 ) }} {{ trans.get('labels.frontend.formation.mb') }}
                )</router-link>
