          ...
          <div class="row">
            ...
            <div v-for="bundle in bundles" :key="bundle.id" class="col-md-4">
              <div class="best-crs-pic-text position-relative">
                <div class="best-crs-pic position-relative" :style="bundle.image ? 'background-image: url(' + ('storage/uploads/fmts/' + formation.image.name | appasset) : '' +')'">
                  ...
                  <div class="crs-details-btn">
                    <router-link :to="{ name:'bundles.show', params:{ slug: bundle.slug}}">{{ trans.get('labels.frontend.formation.bundle_detail')}}
                ...
                <div class="best-crs-text">
                  <div class="crs-title mb20 headline position-relative">
                    <h3>
                      <router-link :to="{ name: 'bundles.show', params:{ slug: bundle.slug }}">{{bundle.title}}</router-link>
                    </h3>
                  </div>
                  <div class="crs-meta">
                    <span class="crs-category">
                      <router-link :to="{ name:'formations.category', params: {category: bundle.category.slug }}">{{bundle.category.name}}</router-link>
                    </span>
                    <span class="crs-author">
                      <a href="#">{{ bundle.students.length }}
                        {{trans.get('labels.frontend.formation.students')}}</a></span>
                    <span v-if="bundle.formations.length > 1" class="crs-author me-0">{{ bundle.formations.length }}
