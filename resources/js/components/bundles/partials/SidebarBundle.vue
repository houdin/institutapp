  ...
  <div class="side-bar">
    <div class="crs-side-bar-widget">
      ...
      <template v-if="!purchased">
        ...
        <button v-if="$filters.Auth.check() === true && $filters.Auth.hasrole('student') === true && $filters.Session.cartSession(formation.id, 'formation') === true" class="btn genius-btn btn-block text-center my-2 text-uppercase  btn-success text-white " type="submit">{{ trans.get('labels.frontend.formation.added_to_cart') }}
        </button>
        <template v-else-if="$filters.Auth.check() !== true">

          <a v-if="formation.free == 1" id="openLoginModal" class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  " data-target="#myModal" href="#">{{ trans.get('labels.frontend.formation.get_now') }} <i class="fas fa-caret-right"></i></a>
          <template v-else>
      ...
      <template v-else>
        ...
        <router-link v-if="continueFormation !== null" :to="{ name: 'modules.show', params: { id: formation.id, slug: continueFormation.model.slug}}" class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  ">{{ trans.get('labels.frontend.formation.continue_formation') }}
