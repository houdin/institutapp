        ...
        <div class="post-categori ul-li-block">
          <ul>
            ...
            <template v-if="categories[0]">
              ...
              <li v-for="(item, index) in categories" :key="index" :class="{'cat-item': true, 'active': ((category !== null && Object.keys(category).length > 0) && (item.slug === category.slug) )}">
      ...
      <div v-if="popularTags[0]" class="side-bar-widget">
        ...
        <div class="tag-clouds ul-li">
          <ul>
            ...
            <li v-for="(item, index) in popularTags" :key="index" :class="(tagsSlug(props.tags).indexOf(item.slug) >= 0 ) ? 'active' : ''">
