<template>
  <div class="row">
    <div class="col-sm-9">
      <!-- <order-information v-if="current === 1" :cart="cart" :order="order">
      </order-information>
      <user-address v-if="current === 2" :isForm="true" :order_id="order.id">
      </user-address>
      <order-payment v-if="current === 3" :cart="cart" :order="order">
      </order-payment>
      <user-order v-if="current === 4" :order_id="order.id"> </user-order> -->
      <router-view name="OrderInformation" v-if="current === 1" :cart="cart" :order="order"></router-view>
      <router-view name="UserAddress" v-if="current === 2" :isForm="true" :order_id="order.id"></router-view>
      <router-view name="OrderPayment" v-if="current === 3" :cart="cart" :order="order"></router-view>
      <router-view name="UserOrder" v-if="current === 4" :order_id="order.id"></router-view>
    </div>
    <!-- /.col -->
    <div class="col-sm-3">
...
export default {
  ...
  created() {
    Event.$on("user-pick-address", (response) => this.createOrder(response));

    Event.$on("user-paid-for-order", (order) => (this.order = order));
  },
