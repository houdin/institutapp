<template>
  ...
  <step-progress-bar :list="steps" :current="current" @update-current="updateCurrent">

  </step-progress-bar>
  <div class="order-wrapper">
    <order-details :current="current" :cart="cart" :user_order="user_order" @address-is-complete="steps.Address.complete = true">

    </order-details>
    <div class="row">
      <div class="pull-left" v-if="notFirst()">
        <a class="btn btn-primary" @click="updateCurrent(getPrevious().step)">
          <i class="fa fa-arrow-left" aria-hidden="true"></i>
          {{ getPrevious().name }}
        </a>
      </div>
      <div class="pull-right" v-if="notLast()">
        <a :class="{'btn btn-success' : true, 'disabled' : (!isComplete())}" @click="updateCurrent(getNext().step)">
          {{ getNext().name }}
...
export default {
  ...
  computed: {
    cart: {
      get() {
        let information = this.$store.state.Cart.information;
